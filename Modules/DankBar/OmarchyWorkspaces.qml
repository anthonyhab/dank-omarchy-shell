...
Rectangle {
    ...
    function getWorkspaceIcons(slot) {
        ...
        wins.forEach((w, i) => {
                         ...
                         if (slot.kind === "niri") {
                             ...
                             const winWorkspace = normalizeWorkspaceId(hyprToplevel && hyprToplevel.workspace ? hyprToplevel.workspace.id : undefined, hyprToplevel && hyprToplevel.workspace ? hyprToplevel.workspace.name : undefined)
    ...
    function shouldKeepHyprlandOverflowSlot(slot, minimumVisible, hyprlandToplevels, windows, targets) {
        ...
        for (var i = 0; i < preserveTargets.length; ++i) {
    ...
    function hyprlandSlotHasWindows(slot, hyprlandToplevels, windows) {
        ...
        for (var i = 0; i < hyprToplevelList.length; ++i) {
    ...
    property int maxWorkspaceIndex: {
        ...
        for (var i = 0; i < workspaceSlots.length; ++i) {
        ...
        Repeater {
            ...
            Rectangle {
                ...
                Timer {
                    id: dataUpdateTimer
                    interval: 50
                    onTriggered: applyDataUpdate()
                }

                Component.onCompleted: requestDataUpdate()
                Component.onDestruction: root.updateActiveHighlight()
                onSlotChanged: requestDataUpdate()
    ...
    Connections {
        ...
        function onRawEvent(event) {
            if (event.name === "workspace" || event.name === "createworkspace" || event.name === "destroyworkspace" || event.name === "monitoradded" || event.name === "monitorremoved" || event.name === "focusedmon") {
                root.refreshWorkspaceSlots()
            }
        }
    }
